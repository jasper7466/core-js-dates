...
function getNextFriday(date) {
  ...
  const targetDay = 5;
  let offset = (targetDay + 7 - currentDay) % 7;
...
function getCountWeekendsInMonth(month, year) {
  ...
  const targetDays = [6, 0];
  let count = 0;

  for (let i = 1; i <= daysInMonth; i += 1) {
    if (targetDays.includes(new Date(year, month - 1, i).getDay())) {
      count += 1;
    }
  }
...
function getNextFridayThe13th(date) {
  ...
  if (dateCopy.getUTCDay() === targetWeekDay) {
    dateCopy.setUTCDate(dateCopy.getUTCDate() + 7);
  } else {
    dateCopy.setUTCDate(dateCopy.getUTCDate() + 5 - dateCopy.getUTCDay());
  }

  while (dateCopy.getUTCDate() !== 13) {
    dateCopy.setUTCDate(dateCopy.getUTCDate() + 7);
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const to = parseDate(period.end);
  const schedule = [];

  while (from <= to) {
    for (let i = 0; i < countWorkDays; i += 1) {
      schedule.push(encodeDate(from));
    ...
    from.setDate(from.getDate() + countOffDays);
  }
...
function isLeapYear(date) {
  ...
  const mult4 = year % 4 === 0;
  const mult100 = year % 100 === 0;
  const mult400 = year % 400 === 0;
